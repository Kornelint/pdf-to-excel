...
st.markdown(
    ...
    2. Na podstawie wykrytego układu (lay‐outu) automatycznie wybiera odpowiedni parser:
       ...
       - **Układ E**: linie zaczynające się od Lp i nazwy, potem ilość, a poniżej „Kod kres.: <EAN>”.  
         (Przykłady: pliki typu `Gussto wola park.pdf` czy `Zamówienie nr ZD 0220/25.pdf`.)  
...
def parse_layout_d(all_lines: list[str]) -> pd.DataFrame:
    ...
    - Name pozostaje pusty, bo w tego typu PDF-ach często nazwa nie jest w głównej linii.
    """
    products = []
    pattern = re.compile(
...
def parse_layout_e(all_lines: list[str]) -> pd.DataFrame:
    ...
    a poniżej (ewentualnie po liniach ARAxxxx) znajduje się linia „Kod kres.: <EAN>”.

    Przykład sekwencji w all_lines:
      1 CANAGAN Kot 0,375kg 8 szt. …
      ARA000585
      Kod kres.: 5029040013097
      2 CANAGAN Kot 0,375kg SCOTTISH 8 szt. …
      Run Turkey
      ARA000613
    ...
    Logika:
      ...
      - Po linii z Lp zbieramy kolejne wiersze aż do znalezienia „Kod kres.:”:
        ...
        • W przeciwnym wypadku traktujemy jako kontynuację nazwy.
      - Gdy napotkamy „Kod kres.: <EAN>”, wyciągamy EAN i kończymy jedną pozycję.
    """
    products = []
    i = 0
    # Wzorzec: Lp na początku, potem nazwa, potem ilość przed "szt."
    ...
    while i < len(all_lines):
        ...
        if m:
            ...
            while j < len(all_lines):
                ...
                # Jeśli linia zaczyna się od "Kod kres.:", wyciągamy EAN i kończymy ten blok
                ...
                # Jeśli linia to czysty alfanumeryczny kod (np. "ARA000613"), pomijamy
            ...
            # Skaczemy do indeksu 'j', bo już przetworzyliśmy do tego momentu linie
            i = j
        else:
            i += 1
...
def parse_layout_c(all_lines: list[str]) -> pd.DataFrame:
    ...
    Przykład w all_lines:
      5029040012366
      3
      Nazwa Produktu
      szt.
      12
      Kod kres.: …   (ewentualnie "Kod kres.:" może być oddzielnie, ale to rzadziej)
    """
    idx_lp = []
    for i in range(len(all_lines) - 1):
        if re.fullmatch(r"\d+", all_lines[i]):
            nxt = all_lines[i + 1]
            # jeśli w kolejnej linii pojawia się fragment nazwy (litery), to to jest Lp
...
def parse_layout_a(all_lines: list[str]) -> pd.DataFrame:
    ...
    Przykład fragmentu all_lines:
      1
      CANAGAN Kot … (czysty tekst nazwy)
      … (może być kilka linii z nazwą)
      8
      szt.
      Kod kres.: 5029040013097
      (następna pozycja)
    ...
    for idx, lp_idx in enumerate(idx_lp):
        ...
        # spośród linii z "Kod kres.:" bierzemy tę, której indeks mieści się między poprzednim Lp a następnym
        ...
        # Szukamy w obrębie bloku od Lp do kolejnego Lp liczby przed "szt."
        ...
        # Po id znalezieniu qty_idx zbieramy jeszcze linie nazwy, aż do "Kod kres.:"
...
# 3) Wyciągnięcie tekstu za pomocą pdfplumber
all_lines = extract_text_with_pdfplumber(pdf_bytes)

# 4) Sprawdź, czy udało się znaleźć w tekście choć jeden 13-cyfrowy EAN
...
# 5) Wykrywanie układu E – Lp + nazwa + ilość w jednej linii oraz obecność „Kod kres.:”
...
is_layout_e = bool(pattern_e.match("\n".join(all_lines))) and has_kod_kres

# 6) Wykrywanie układu D – EAN + ilość w tej samej linii (bez Lp)
...
# 7) Wykrywanie układu B – każda pozycja w jednej linii: Lp + EAN + nazwa + ilość,0xx szt.
...
# 8) Wykrywanie układu C – istnieje czysty 13-cyfrowy EAN w oddzielnej linii, ale to nie układ B ani D
...
# 9) Na podstawie wykrycia układu wybierz parser
if is_layout_e:
...
# 10) Odfiltruj wiersze, które nie mają ilości (jeśli kolumna istnieje)
if "Quantity" in df.columns:
    df = df.dropna(subset=["Quantity"]).reset_index(drop=True)

# 11) Sprawdź, czy po parsowaniu w ogóle coś znaleziono
if df.empty:
    st.error(
...
# 12) Wyświetl dataframe w Streamlit
