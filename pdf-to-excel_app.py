...
st.markdown(
    ...
    3. Gdy już mamy listę wierszy (`all_lines`), wykrywamy układ:
       - **Układ E**: pozycje zaczynające się od numeru Lp, potem nazwa, ilość, a poniżej „Kod kres.: <EAN>” (z opcjonalnymi wierszami kontynuacji nazwy i pomiędzy „ARA…”).  
       - **Układ D**: proste linie zawierające tylko EAN i ilość, np.  
         `5029040012366 Nazwa Produktu 96,00 szt.` albo `5029040012366 96,00 szt.`  
       - **Układ B**: każda pozycja w jednej linii:  
         `<Lp> <EAN(13)> <pełna nazwa> <ilość>,<xx> szt …`  
       - **Układ C**: czysty 13-cyfrowy EAN w osobnej linii, potem Lp w osobnej linii, potem nazwa, „szt.” i ilość w kolejnych wierszach.  
       - **Układ A**: „Kod kres.: <EAN>” w osobnej linii, Lp w osobnej linii, fragmenty nazwy przed/po kolumnie cen/ilości.  
    4. W zależności od wykrytego układu wywołujemy odpowiedni parser (E, D, B, C lub A).
...
def parse_layout_e(all_lines: list[str]) -> pd.DataFrame:
    """
    Parser dla układu E – linie zaczynające się od Lp i nazwy, potem ilość w tym samym wierszu,
    a poniżej mogą się pojawić wiersze z katalogiem (ARA...) lub kontynuacje nazwy,
    a następnie linia „Kod kres.: <EAN>”.
    
    Przykład sekwencji w all_lines:
      1 CANAGAN Kot 375g Country Game 6 szt. 20,60 123,60 133,49
      ARA000585
      Kod kres.: 5029040012892
      2 CANAGAN Kot 375g Dental Free 5 szt. 18,68 93,40 100,87
      Run Turkey
      ARA000613
    ...
    Logika:
      - Szukamy linii rozpoczynającej się od Lp (cyfra) i zawierającej „<ilość> szt.”.
      - Grupa 1: numer Lp; grupa 2: początkowa część nazwy; grupa 3: ilość.
      - Następnie zbieramy kolejne wiersze aż do momentu napotkania „Kod kres.:” – 
        wszystkie wiersze niebędące alfanumerycznym „ARA…” są traktowane jako kontynuacja nazwy.
      - Gdy napotkamy „Kod kres.: <EAN>”, wyciągamy EAN i kończymy zbieranie dla tego Lp.
    """
    products = []
    i = 0
    # Wzorzec: Lp na początku, potem nazwa, potem liczba (ilość) przed "szt."
    ...
    while i < len(all_lines):
        ...
        if m:
            ...
            Quantity_val = int(m.group(3))
            name_parts = [initial_name]
            barcode = None

            # Przechodzimy do kolejnych wierszy, szukając linii z "Kod kres.:"
            j = i + 1
            while j < len(all_lines):
                next_ln = all_lines[j]

                # Jeśli linia zaczyna się od "Kod kres.:", wyciągamy EAN i kończymy
                ...
                # Jeśli linia to alfanumeryczny ciąg bez spacji (np. "ARA000613"), pomijamy
                if re.fullmatch(r"[A-Za-z0-9]+", next_ln):
                    j += 1
                    continue

                # W przeciwnym razie traktujemy to jako kontynuację nazwy
                name_parts.append(next_ln.strip())
                j += 1

            # Złączone fragmenty nazwy
            ...
            # Przeskakujemy i ustawiamy i = j, aby pominąć już przetworzone linie
            i = j
        else:
            i += 1
...
# 3) Ekstrakcja tekstu przy pomocy pdfplumber
...
# 6) Wykryj układ E – Lp + nazwa + ilość w jednej linii + poniżej „Kod kres.: <EAN>”
...
# 10) Parsuj w zależności od wykrytego układu
if is_layout_e:
